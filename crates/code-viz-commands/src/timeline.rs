...
pub async fn generate_timeline(file_path: &Path, git: &impl GitProvider) -> Result<TimelineResult> {
    ...
    for commit in history {
        let content = git
            .get_file_content_at_revision(file_path, &commit.sha)
            .await?;
        ...
        let file_metrics =
            code_viz_core::metrics::calculate_metrics(file_path, &content, parser.as_ref(), None)?;
