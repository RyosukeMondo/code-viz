...
fn create_test_repository() -> MockFileSystem {
    MockFileSystem::new()
        .with_file("/test_repo/src/main.rs", r#"
fn main() {
    println!("Hello, world!");
}
...
"#)
        .with_file("/test_repo/src/lib.rs", r#"
...
"#)
        .with_file("/test_repo/src/utils.ts", r#"
...
fn create_malformed_repository() -> MockFileSystem {
    MockFileSystem::new()
        .with_file("/test_repo/src/good.rs", r#"
fn good_function() {
    println!("This is fine");
}
"#)
        .with_file("/test_repo/src/broken.rs", r#"
fn broken_function( {
    // Missing closing brace and param list
"#)
        .with_file("/test_repo/src/another_good.ts", r#"
function goodTs() {
...
fn create_multi_language_repository() -> MockFileSystem {
    MockFileSystem::new()
        .with_file("/multi_lang_repo/src/main.rs", r#"
fn main() {
    println!("Rust code");
}
"#)
        .with_file("/multi_lang_repo/src/app.ts", r#"
function main() {
    console.log("TypeScript code");
}
"#)
        .with_file("/multi_lang_repo/src/script.py", r#"
def main():
    print("Python code")
"#)
        .with_file("/multi_lang_repo/src/main.go", r#"
package main
...
async fn test_full_analysis_pipeline() {
    ...
    assert!(result.duplication.is_some(), "Duplication analysis should be present");

    // Verify hotspot analysis ran
    assert!(result.hotspot_analysis.is_some(), "Hotspot analysis should be present");
...
async fn test_partial_analysis() {
    ...
    assert!(result.duplication.is_none(), "Duplication analysis should be absent");
    assert!(result.coverage_analysis.is_none(), "Coverage analysis should be absent");
}
...
async fn test_error_recovery_malformed_files() {
    ...
    assert!(result.is_ok(), "Analysis should handle malformed files gracefully");
    let result = result.unwrap();
    ...
    let rust_files: Vec<_> = result.files.iter()
        .filter(|f| f.language == "rust")
        .collect();
    assert_eq!(rust_files.len(), 2, "Should process both Rust files (parser handles errors)");

    // Verify TypeScript file was also processed
    let ts_files: Vec<_> = result.files.iter()
...
async fn test_large_repository() {
    ...
    assert_eq!(result.summary.total_files, 50, "Summary should reflect 50 files");

    // Performance check: should complete in reasonable time (< 5 seconds)
    assert!(duration.as_secs() < 5, "Large repository analysis should complete in < 5 seconds, took {:?}", duration);

    // Verify progress reporting
    let progress_events = ctx.get_events_by_name("progress");
    assert!(progress_events.len() > 0, "Should emit progress events for large repo");
}
...
async fn test_empty_repository() {
    ...
    assert!(result.is_ok(), "Empty repository should be handled gracefully");
    let result = result.unwrap();
    ...
    assert_eq!(result.summary.total_functions, 0, "Summary should show 0 functions");
}
...
async fn test_multi_language_repository() {
    ...
    let languages: std::collections::HashSet<_> = result.files
        .iter()
        .map(|f| f.language.as_str())
        .collect();

    assert!(languages.contains("rust"), "Should detect Rust files");
    assert!(languages.contains("typescript"), "Should detect TypeScript files");
    ...
    let ts_count = result.files.iter().filter(|f| f.language == "typescript").count();
    let py_count = result.files.iter().filter(|f| f.language == "python").count();
...
async fn test_analysis_determinism() {
    ...
    // Run analysis twice
    let result1 = analyze_repository(
        repo_path,
        ctx.clone(),
        fs.clone(),
        &git,
    ...
    let result2 = analyze_repository(
        repo_path,
        ctx.clone(),
        fs.clone(),
        &git,
    ...
    assert_eq!(result1.files.len(), result2.files.len(), "File count should be deterministic");
    assert_eq!(result1.summary.total_loc, result2.summary.total_loc, "LOC count should be deterministic");
    assert_eq!(result1.summary.total_functions, result2.summary.total_functions, "Function count should be deterministic");
}
